...
public class SourcePositionMapper {
    ...
    public SourceRangeId map(SourceRange range) {
        SourcePositionId start = map(range.getStart());
        SourcePositionId end = map(range.getEnd());
        return SourceRangeId.create(start, end);
    }
